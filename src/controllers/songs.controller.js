...
const createSong = async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: 'Error creating song',error: error.message });
    }
  };
...
console.log(q);
  try {
    ...
    res.status(500).json({ error: 'Error searching songs' ,error: error.message });
  }
};
...
const deleteSongById = async (req, res) => {
    ...
    try {
      ...
      if (song.artistId !== userId) {
        return res.status(403).json({ error: 'Permission denied. Only the owner can delete the song.' });
      }
